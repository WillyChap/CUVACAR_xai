...
def run_year_rmse(p, config, input_shape, forcing_shape, output_shape, device, model_name=None, init_noise=None, save_append=None):
    ...
    for lat_ in lats:
        for lon_ in lons: 
            for lev_ in levs: 
                integrated_grads = get_integrated_gradients(x, model, baseline=None, num_steps=50, top_pred_idx=[lev_,0,lat_,lon_])
                arr = integrated_grads.detach().cpu().numpy()   # detach (no grad), move to CPU, convert
                end_time = time.perf_counter()
                elapsed_time = end_time - start_time
                print(f"IG - Elapsed time: {elapsed_time:.4f} seconds")
                np.save(f"/glade/derecho/scratch/wchapman/sphereofinf/tensor_lev{lev_:05}_lat{lat_:05}_lon{lon_:05}.npy", arr)
